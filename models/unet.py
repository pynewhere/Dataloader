...
class UNet(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, zero_output=False, dim=48): #假设输入3×512×512
    ...
    def forward(self, x):
        ...
        # Encoder
        pool1 = self.encode_block_1(x)   # 48×256×256
        pool2 = self.encode_block_2(pool1) # 48×128×128
        pool3 = self.encode_block_3(pool2) # 48×64×64
        pool4 = self.encode_block_4(pool3) # 48×32×32
        pool5 = self.encode_block_5(pool4) # 48×16×16
        encoded = self.encode_block_6(pool5) # 48×16×16

        # Decoder
        upsample5 = self.decode_block_6(encoded) # 48×32×32
        concat5 = torch.cat((upsample5, pool4), dim=1) # 96×32×32
        upsample4 = self.decode_block_5(concat5) # 96×64×64
        concat4 = torch.cat((upsample4, pool3), dim=1) # 96×64×64
        upsample3 = self.decode_block_4(concat4) # 96×128×128
        concat3 = torch.cat((upsample3, pool2), dim=1) # 96×128×128
        upsample2 = self.decode_block_3(concat3) # 96×256×256
        concat2 = torch.cat((upsample2, pool1), dim=1)  # 96×256×256
        upsample1 = self.decode_block_2(concat2) # 96×512×512
        concat1 = torch.cat((upsample1, x), dim=1) # 96×512×512
        x = self.decode_block_1(concat1) # 48×512×512

        x = self.output_conv(x)   # 3×512×512
